...
async function fetchEmails(tokens, maxResults = 100) {
  const { client, newTokens } = await getAuthedClient(tokens);
  const gmail = google.gmail({ version: 'v1', auth: client });

  // Build today's date query (Gmail format: after:YYYY/M/D)
  const now = new Date();
  const todayQuery = `after:${now.getFullYear()}/${now.getMonth() + 1}/${now.getDate()}`;
